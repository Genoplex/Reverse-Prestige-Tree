...
function save(force) {
	...
	localStorage.setItem(modInfo.id, encodeBase64WithChinese(JSON.stringify(player)));
	localStorage.setItem(modInfo.id + "_options", encodeBase64WithChinese(JSON.stringify(options)));
}
...
function load() {
	...
	if (get === null || get === undefined) {
		...
		player = Object.assign(getStartPlayer(), JSON.parse(decodeBase64WithChinese(get)));
		fixSave();
		loadOptions();
	}
...
function loadOptions() {
	...
	if (get2)
		options = Object.assign(getStartOptions(), JSON.parse(decodeBase64WithChinese(get2)));
	else
		options = getStartOptions()
	if (themes.indexOf(options.theme) < 0) theme = "obsidian"
...
function exportSave() {
	//if (NaNalert) return
	let str = encodeBase64WithChinese(JSON.stringify(player));
...
function importSave(imported = undefined, forced = false) {
	...
	try {
		tempPlr = Object.assign(getStartPlayer(), JSON.parse(decodeBase64WithChinese(imported)));
...
function encodeBase64WithChinese(input) {
	let encode = encodeURI(input)
	let base64 = btoa(encode)
	return base64
}

function decodeBase64WithChinese(input) {
